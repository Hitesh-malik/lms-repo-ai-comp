...
import { useUpdateQuizQuestionMutation } from "@/hooks/useCourseMutations";
import toast from "react-hot-toast";
import { getApiErrorMessage } from "@/lib/utils";
import { Loader2 } from "lucide-react";
import type { QuizQuestion } from "@/services/courseApi";
...
function getInitialValues(question: QuizQuestion): EditQuestionFormValues {
  return {
...
export default function EditQuestionForm({
  ...
  const updateQuestionMutation = useUpdateQuizQuestionMutation(question.id, quizId);

  const handleSubmit = async (values: EditQuestionFormValues) => {
    try {
  ...
  return (
    <div className="flex flex-col gap-5 py-2 max-h-[85vh] overflow-y-auto">
      <div>
        <h3 className="text-xl font-semibold text-slate-900">Edit Question</h3>
        <p className="text-sm text-slate-600 mt-1">Update the multiple choice question.</p>
      </div>

      <Formik<EditQuestionFormValues>
